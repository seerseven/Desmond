...
var HeaderDrawer = class extends MenuDrawer {
  ...
  closeMenuDrawer(event2, elementToFocus) {
  ...
  constructor() {
    ...
    if (this.classList.contains("media-modal")) {
      ...
      this.addEventListener("click", (event2) => {
        if (event2.target.nodeName === "MODAL-DIALOG")
          this.hide();
      });
    }
  }
...
var DeferredMedia = class extends HTMLElement {
  constructor() {
    super();
...
var SliderComponent = class extends HTMLElement {
  ...
  onButtonClick(event2) {
    event2.preventDefault();
    const step = event2.currentTarget.dataset.step || 1;
    this.slideScrollPosition = event2.currentTarget.name === "next" ? this.slider.scrollLeft + step * this.sliderItemOffset : this.slider.scrollLeft - step * this.sliderItemOffset;
...
var SlideshowComponent = class extends SliderComponent {
  constructor() {
    super();
  ...
  setAutoPlay() {
    ...
    this.autoplaySpeed = this.slider.dataset.speed * 1e3;
  ...
  onButtonClick(event2) {
    super.onButtonClick(event2);
    const isFirstSlide = this.currentPage === 1;
    const isLastSlide = this.currentPage === this.sliderItemsToShow.length;
    if (!isFirstSlide && !isLastSlide)
      return;
    if (isFirstSlide && event2.currentTarget.name === "previous") {
      this.slideScrollPosition = this.slider.scrollLeft + this.sliderFirstItemNode.clientWidth * this.sliderItemsToShow.length;
    } else if (isLastSlide && event2.currentTarget.name === "next") {
  ...
  focusOutHandling(event2) {
    const focusedOnAutoplayButton = event2.target === this.sliderAutoplayButton || this.sliderAutoplayButton.contains(event2.target);
    if (!this.autoplayButtonIsSetToPlay || focusedOnAutoplayButton)
      return;
    this.play();
  }
  focusInHandling(event2) {
    const focusedOnAutoplayButton = event2.target === this.sliderAutoplayButton || this.sliderAutoplayButton.contains(event2.target);
  ...
  linkToSlide(event2) {
    event2.preventDefault();
    const slideScrollPosition = this.slider.scrollLeft + this.sliderFirstItemNode.clientWidth * (this.sliderControlLinksArray.indexOf(event2.currentTarget) + 1 - this.currentPage);
    this.slider.scrollTo({
      left: slideScrollPosition
    });
  }
...
var DetailsModal = class extends HTMLElement {
  constructor() {
    ...
    this.detailsContainer.addEventListener("keyup", (event2) => event2.code.toUpperCase() === "ESCAPE" && this.close());
...
var shopifyElements = function enableElements() {
  const shopifyModules = {
    ...
    gallery: function() {
      if (!customElements.get("media-gallery")) {
        customElements.define("media-gallery", class MediaGallery extends HTMLElement {
          ...
          onSlideChanged(event2) {
            const thumbnail = this.elements.thumbnails.querySelector(`[data-target="${event2.detail.currentElement.dataset.mediaId}"]`);
    ...
    facets: function() {
      class FacetFiltersForm extends HTMLElement {
        ...
        static setListeners() {
          const onHistoryChange = (event2) => {
            const searchParams = event2.state ? event2.state.searchParams : FacetFiltersForm.searchParamsInitial;
        ...
        static renderPage(searchParams, event2, updateURLHash = true) {
          ...
          sections.forEach((section) => {
            ...
            FacetFiltersForm.filterData.some(filterDataUrl) ? FacetFiltersForm.renderSectionFromCache(filterDataUrl, event2) : FacetFiltersForm.renderSectionFromFetch(url, event2);
          });
          if (updateURLHash)
        ...
        static renderSectionFromFetch(url, event2) {
          fetch(url).then((response) => response.text()).then((responseText) => {
            ...
            FacetFiltersForm.renderFilters(html, event2);
        ...
        static renderFilters(html, event2) {
          ...
          if (countsToRender)
            FacetFiltersForm.renderCounts(countsToRender, event2.target.closest(".js-filter"));
        }
        ...
        onSubmitHandler(event2) {
          event2.preventDefault();
          const formData = new FormData(event2.target.closest("form"));
          const searchParams = new URLSearchParams(formData).toString();
          FacetFiltersForm.renderPage(searchParams, event2);
        }
      ...
      class PriceRange extends HTMLElement {
        ...
        onRangeChange(event2) {
          this.adjustToValidValues(event2.currentTarget);
          this.setMinAndMaxValues();
        }
        setMinAndMaxValues() {
      ...
      class FacetRemove extends HTMLElement {
        constructor() {
          super();
          this.querySelector("a").addEventListener("click", (event2) => {
            event2.preventDefault();
    ...
    local: function() {
      class LocalizationForm extends HTMLElement {
        ...
        closeSelector(event2) {
          const shouldClose = event2.relatedTarget && event2.relatedTarget.nodeName === "BUTTON";
          if (event2.relatedTarget === null || shouldClose) {
            this.hidePanel();
          }
        }
      }
    ...
    themeEditor: function() {
      document.addEventListener("shopify:block:select", function(event2) {
        const blockSelectedIsSlide = event2.target.classList.contains("slideshow__slide");
        if (!blockSelectedIsSlide)
          return;
        const parentSlideshowComponent = event2.target.closest("slideshow-component");
        parentSlideshowComponent.pause();
        setTimeout(function() {
          parentSlideshowComponent.slider.scrollTo({
            left: event2.target.offsetLeft
          });
        }, 200);
      });
      document.addEventListener("shopify:block:deselect", function(event2) {
        const blockDeselectedIsSlide = event2.target.classList.contains("slideshow__slide");
